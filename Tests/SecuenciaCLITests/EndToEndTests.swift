  ...
  struct EndToEndTests {
    ...
    func simpleTimelineExport() async throws {
      ...
      definition = try resolver.resolve(
        definition: definition, relativeTo: jsonURL.deletingLastPathComponent())
    ...
    func missingJSONFile() async throws {
      ...
      do {
        ...
        #expect(
          errorDescription.contains(nonexistentURL.path)
    ...
    func malformedJSON() async throws {
      ...
      do {
        ...
        #expect(
          String(describing: error).contains("parse") || String(describing: error).contains("JSON")
            || String(describing: error).contains("decode"))
      }
    }
    ...
    func nonexistentMediaFile() async throws {
      ...
      do {
        _ = try resolver.resolve(
          definition: definition, relativeTo: tempURL.deletingLastPathComponent())
        Issue.record("Expected error for nonexistent media file")
      } catch {
        // Error should contain the resolved absolute path
        let errorDescription = String(describing: error)
        #expect(
          errorDescription.contains("nonexistent.mov")
    ...
    func clipNoDurationNoFile() async throws {
      ...
      do {
        ...
        #expect(
          String(describing: error).contains("file")
            || String(describing: error).contains("required"))
      }
    }
    ...
    func invalidTimeString() async throws {
      ...
      do {
        ...
        let errorDescription = String(describing: error)
        #expect(
          errorDescription.contains("not-a-time") || errorDescription.contains("invalid")
