...
func swiftSecuenciaExporterExportsClipMarkers() throws {
  ...
  #expect(
    fcpxml.contains("<marker"), "FCPXML must contain <marker element -- PipelineNeo exports markers"
  )
...
func swiftSecuenciaExporterExportsClipKeywords() throws {
  ...
  #expect(
    fcpxml.contains("<keyword"),
    "FCPXML must contain <keyword element -- PipelineNeo exports keywords")
...
func swiftSecuenciaExporterExportsClipRatings() throws {
  ...
  #expect(
    fcpxml.contains("<rating"), "FCPXML must contain <rating element -- PipelineNeo exports ratings"
  )
...
func swiftSecuenciaExporterExportsAllMetadataTypes() throws {
  ...
  clip1.markers.append(
    Marker(
      start: Timecode(seconds: 10),
      value: "Scene Change"
    ))
  clip1.keywords.append(
    Keyword(
      start: Timecode.zero,
      duration: Timecode(seconds: 30),
      value: "Interview"
  ...
  clip2.ratings.append(
    Rating(
      start: Timecode.zero,
      duration: Timecode(seconds: 40),
      value: .favorite
    ))
  clip2.chapterMarkers.append(
    ChapterMarker(
      start: Timecode.zero,
      value: "Chapter 1"
    ))
...
func swiftSecuenciaExporterOmitsEmptyMetadata() throws {
  ...
  #expect(
    !fcpxml.contains("<chapter-marker"), "No chapter markers should appear for empty-metadata clip")
}
...
func swiftSecuenciaExporterMetadataPassesDTDValidation() throws {
  ...
  clip.keywords.append(
    Keyword(start: Timecode.zero, duration: Timecode(seconds: 30), value: "Test"))
  clip.ratings.append(
    Rating(start: Timecode.zero, duration: Timecode(seconds: 30), value: .favorite))
  var meta = Metadata()
  meta.setReel("A001")
  clip.metadata = meta
  timeline.insertClip(clip, at: .zero, lane: 0)
  ...
  // DEBUG: Print first few lines to see XML declaration
  let lines = fcpxml.components(separatedBy: "\n").prefix(5)
  print("DEBUG XML Declaration:")
  for line in lines {
    print("  \(line)")
  }
  ...
  if !result.isValid {
    Issue.record(
      "DTD validation failed for metadata FCPXML:\n\(result.errors.joined(separator: "\n"))")
  }
