...
@Test func multiLaneTimelinePassesDTDValidation() async throws {
  ...
  // Lane -1 (audio below)
  timeline.insertClip(
    TimelineClip(assetStorageId: audioAsset.id, duration: Timecode(seconds: 30)), at: .zero,
    lane: -1)

  // Lane 1 (connected above)
  timeline.insertClip(
    TimelineClip(assetStorageId: videoAsset.id, duration: Timecode(seconds: 20)),
...
@Test func differentVideoFormatsPassDTDValidation() async throws {
  ...
  for format in formats {
    ...
    if !result.isValid {
      Issue.record(
        "DTD validation failed for format \(format.fcpxmlFormatName):\n\(result.errors.joined(separator: "\n"))"
      )
    }
...
@Test func validateAgainstMultipleDTDVersions() async throws {
  ...
  for version in versions {
    ...
    let validator = FCPXMLDTDValidator()
    let dtdFilename =
      "Fixtures/FCPXMLv\(version.stringValue.replacingOccurrences(of: ".", with: "_"))"
    ...
    if !result.isValid {
      Issue.record(
        "DTD validation failed for version \(versionStr):\n\(result.errors.joined(separator: "\n"))"
      )
    }
