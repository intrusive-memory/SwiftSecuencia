...
extension TimelineClip {
  ...
  public func validateAsset(in modelContext: SwiftData.ModelContext) throws -> TypedDataStorage {
    ...
    guard let asset = try modelContext.fetch(descriptor).first else {
      throw TimelineError.invalidAssetReference(
        storageId: assetStorageId, reason: "Asset not found")
    }
    ...
    if lane < 0 {
      ...
      guard
        mimeType.hasPrefix("video/") || mimeType.hasPrefix("image/") || mimeType.hasPrefix("audio/")
      else {
        throw TimelineError.invalidFormat(
          reason:
            "Clip on lane \(lane) expects video/*, image/*, or audio/* but asset has MIME type '\(mimeType)'"
        )
      }
    }
