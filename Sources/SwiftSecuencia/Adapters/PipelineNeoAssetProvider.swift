...
public struct PipelineNeoAssetProvider {
    ...
    public func exportAsset(
        ...
        guard let resourceID = resourceMap.assetResourceID(for: uuid) else {
            ...
            return try buildExportAsset(for: uuid, resourceID: uuid.uuidString, relativePath: relativePath)
        }
    ...
    public func exportAssetOrNil(
        ...
        do {
            ...
            print("PipelineNeoAssetProvider: Failed to convert asset \(uuid): \(error.localizedDescription)")
            return nil
        }
    }
