...
//
  ...
  public struct GenerateFCPXMLBundleIntent: AppIntent {
    ...
    public static let description = IntentDescription(
      stringLiteral:
        "Create a Final Cut Pro XML bundle from screenplay elements with audio. Includes timeline structure and embedded media files."
    )
    ...
    private func createTimeline(
      ...
      for element in reference.elements where element.isDialogue {
        ...
        if let audio = try? findAudio(for: element, in: modelContext) {
          let duration =
            audio.durationSeconds.map { Timecode(seconds: $0) }
            ?? Timecode(seconds: defaultClipDuration)

          let clip = TimelineClip(
            name:
              "\(element.characterName ?? "Character"): \(String(element.elementText.prefix(30)))...",
