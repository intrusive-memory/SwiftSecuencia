  ...
  public struct SwiftSecuenciaBundleExporter {
    ...
    public func exportBundle(
      ...
      do {
        ...
        throw FCPXMLBundleExportError.bundleCreationFailed(
          reason:
            "Could not create bundle directory at \(bundleURL.path): \(error.localizedDescription)"
        )
      }
    ...
    private func updateSrcAttribute(
      ...
      guard let srcAttr = element.attribute(forName: "src"),
        let currentSrc = srcAttr.stringValue
      else {
        return
      }
    ...
    private func verifyLibraryNameRemoved(from infoFCPXMLURL: URL) throws {
      ...
      do {
        ...
        throw FCPXMLBundleExportError.writeFailed(
          reason:
            "Could not read Info.fcpxml for library name verification: \(error.localizedDescription)"
        )
      }
      ...
      do {
        ...
        throw FCPXMLBundleExportError.writeFailed(
          reason:
            "Could not parse Info.fcpxml as XMLDocument during library name verification: \(error.localizedDescription)"
        )
      }
      ...
      if modified {
        ...
        do {
          ...
          throw FCPXMLBundleExportError.writeFailed(
            reason:
              "Could not rewrite Info.fcpxml after library name removal: \(error.localizedDescription)"
          )
        }
      }
    }
    ...
    private func copyMediaFiles(
      ...
      for uuid in uniqueAssetUUIDs {
        guard let resourceID = resourceMap.assetResourceID(for: uuid),
          let relativePath = relativePathMap[uuid]
        else {
          continue
        }
