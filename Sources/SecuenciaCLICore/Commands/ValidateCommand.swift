...
public struct Validate: AsyncParsableCommand {
    ...
    public mutating func run() async throws {
        ...
        print("  Format: \(probed.timeline.format.width)×\(probed.timeline.format.height) @ \(probed.timeline.format.frameRate) fps")
        ...
        for clip in probed.clips where clip.duration != nil {
            if let duration = try? timeParser.parse(clip.duration!), duration.seconds < 0.1 {
                warnings.append("Clip '\(clip.name)' is very short (\(String(format: "%.3f", duration.seconds))s)")
            }
        }
        ...
        for (lane, clips) in clipsByLane {
            ...
            for i in 0..<sortedClips.count - 1 {
                ...
                if currentEnd > nextStart.seconds {
                    warnings.append("Clips '\(currentClip.name)' and '\(nextClip.name)' overlap on lane \(lane)")
                }
            }
        }
        ...
        if let primaryClips = clipsByLane[0]?.filter({ $0.type != .marker }) {
            ...
            for i in 0..<sorted.count - 1 {
                ...
                if gap > 0.01 { // Allow small floating point errors
                    warnings.append("Gap of \(String(format: "%.2f", gap))s in primary storyline between '\(currentClip.name)' and '\(nextClip.name)'")
                }
            }
        }
